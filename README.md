...
仅在LLBOT平台测试过，其他机器人未测试，不保证兼容。

## ✨ 功能亮点

- **🚀 双模式上传**：
  - **Base64 模式**：由于兼容性最广，优先使用 Base64 编码发送。适合 30MB 以下的小视频，能有效绕过部分中转服务商对 File API 的限制。
  - **Native Upload 模式**：自动回退机制。对于大视频或 Base64 失败的情况，自动切换至 Gemini 原生 File API（上传->处理流程）。
...
- **� 实时反馈**：在分析过程中提供“正在看视频”的中间反馈，提升用户交互体验。
